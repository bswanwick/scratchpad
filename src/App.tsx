...
import "./App.css";
import Samples, { Sample, SamplesByKey } from "./utils/Samples";

interface PlayableSample extends Sample {
  howlObject: any;
}

const PlayableSamples: PlayableSample[] = Samples.map(
  sample =>
    new Object({
      ...sample,
      howl: new Howl({ src: sample.path })
    }) as PlayableSample
);
...
const App: React.FC = () => {
  ...
  return (
    <div id="App">
      ...
      {PlayableSamples.map((ps, index) => (
        <button onClick={() => ps.howlObject.play()} key={index}>
          {ps.filename}
        </button>
      ))}
    </div>
